  ...
  for (const product of products) {
    ...
    if (url) {
      images.push({
        ...
        categorySlug: Array.isArray(product.categories) && product.categories.length > 0 ? (product.categories[0] as any)?.slug || 'outros' : 'outros',
      })
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 h-full">
                  {slide.products && slide.products.length > 0 ? (
                    slide.products.map((product) => (
                      <Link
                        key={product._id}
                        href={`/produtos/${Array.isArray(product.categories) && product.categories.length > 0 ? (product.categories[0] as any)?.slug || 'outros' : 'outros'}/${product.slug?.current || ''}`}
